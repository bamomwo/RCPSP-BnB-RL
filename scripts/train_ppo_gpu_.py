...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "tb_log_dir": None,  # defaults to <save_path parent>/tb_logs
    "seed": 42,
    "device": "cpu",
}
...
def main() -> None:
    ...
    # --- TensorBoard ---
    tb_log_dir = config.get("tb_log_dir")
    if tb_log_dir is None:
        tb_log_dir = str(save_path.parent / "tb_logs")
    ...
    while global_step < total_env_steps:
        ...
        writer.add_scalar("episode/g_bonus", g_first + g_improve, global_step)
        writer.add_scalar("episode/steps", episode_steps, global_step)
        writer.add_scalar("episode/inc_improvements", stats.incumbent_improvements, global_step)
        writer.add_scalar("episode/time_limit", 1.0 if stats.done_reason == "time_limit" else 0.0, global_step)
        ...
        writer.add_scalar("loss/policy", total_pg_loss / n_updates, global_step)
        writer.add_scalar("loss/value", total_vf_loss / n_updates, global_step)
        writer.add_scalar("loss/entropy", total_ent / n_updates, global_step)
        writer.add_scalar("train/kl", mean_kl, global_step)
        writer.add_scalar("train/explained_var", explained_var, global_step)
        writer.add_scalar("train/ret_std", ret_std, global_step)
        writer.add_scalar("train/ent_coef", ent_coef_now, global_step)
        writer.add_scalar("train/episodes_in_batch", len(episode_records), global_step)
        writer.add_scalar("train/valid_frac", n_valid / n_total if n_total else 0.0, global_step)
        writer.add_scalar("train/kl_stop", 1.0 if early_stop else 0.0, global_step)
    ...
    if eval_paths and optimal_makespans and global_step != last_eval_step:
        ...
        # TensorBoard: final eval metrics
        writer.add_scalar("eval/solved_frac", metrics["solved_frac"], global_step)
        writer.add_scalar("eval/mean_gap", metrics["mean_gap"], global_step)
        writer.add_scalar("eval/mean_nodes", metrics["mean_nodes"], global_step)
